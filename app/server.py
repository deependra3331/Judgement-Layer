...
def call_gemini(system_prompt, user_content, api_key=None):
    ...
    if not api_key:
        raise Exception("Gemini API Key is missing. Please set it in your environment or Settings panel.")
...
class SPAHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/health':
            ...
            self.end_headers()
            self.wfile.write(json.dumps({"status": "ok", "version": "unified"}).encode('utf-8'))
            return
...
if __name__ == '__main__':
    print(f"Judgment Layer — Unified App")
