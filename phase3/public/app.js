...
// 5. Fetch Judgment score breakdown (Phase 3)
async function fetchScore(payload) {
  try {
    ...
    // Mock improved displays (will be replaced by API in Phase 4)
    document.getElementById('improved-output-display').innerHTML = 
      payload.output.replace(/(?:[0-9]{1,2}(?:\.[0-9]+)?%)/g, match => `<span class="highlight-add">${match} (verified)</span>`) + 
